...
test('listen_match is a real scored match exercise in production and QA routes', () => {
  for (const source of [prodSource, qaSource]) {
    ...
    assert.match(source, /orderedMatchColumns/, 'visible audio and translation columns should be shuffled independently from backend pair order')
    assert.match(source, /leftItems\.map/, 'audio cards should render from shuffled left column')
    assert.match(source, /rightItems\.map/, 'translation cards should render from shuffled right column')
