...
test('ExercisesQA exposes session integrity metadata without adding local practice', () => {
  ...
  assert.match(exercisesSource, /const isAwaitingFinish = session\.current_index >= session\.total_count/, 'QA strip should recognize the legitimate post-last-answer finish window')
  assert.match(exercisesSource, /const hasMissingActiveItem = !isAwaitingFinish && session\.current_index >= sessionItemCount/, 'QA blocker should not flag the legitimate finish window as a missing active item')
  assert.match(exercisesSource, /janela ativa:[\s\S]*fim aguardando conclusão[\s\S]*item em progresso/, 'QA strip should expose whether the window is awaiting finish or still in progress')
  ...
  assert.match(exercisesSource, /item\.id renderizado/, 'QA strip should expose the rendered item id')
  assert.match(exercisesSource, /session item\.id/, 'QA strip should expose the expected real-session item id')
  assert.match(exercisesSource, /Origem renderizada/, 'QA strip should expose whether the rendered item comes from the current session or feedback snapshot')
