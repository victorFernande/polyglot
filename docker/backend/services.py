...
class ExerciseService:
    ...
    SESSION_SIZE = 20
    TARGET_ITEMS = 1000
    INCREMENTAL_ITEM_TARGETS = {"de": 1100}
    ...
    def _incremental_review_items(code: str, start_index: int, count: int):
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — descreva sua família ao encerrar a apresentação. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
        ...
        sequence_pairs = [phrases[5], phrases[7], phrases[8], phrases[9]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro filha; depois idade; em seguida casa; por fim descrição",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — feche falando da família. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))

        closing_hint = f"Mini-aula: {unit['goal']} Esta revisão fecha o bloco de 20 questões reais sem ultrapassar o limite da sessão."
        for item in items[session_55_start + 10:]:
            if item["type"] == "listen_build":
                item["hint"] = f"{closing_hint} Ouça a frase, repita em voz alta e monte as palavras na ordem correta."
            elif item["type"] == "listen_match":
                item["hint"] = f"{closing_hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
                item["explanation"] = f"Cada áudio em {name} deve ser ligado à tradução em português dentro da revisão de família."
            elif item["type"] == "sequence_dialogue":
                item["hint"] = f"{closing_hint} Siga a ordem indicada no enunciado e organize apenas as frases no idioma estudado."
            else:
                item["hint"] = closing_hint
            if item["type"] in {"build", "listen_build"}:
                item["explanation"] = f"A frase correta é: “{' '.join(item['answer']['value'])}”."
            elif item["type"] not in {"image_choice", "listen_match", "sequence_dialogue"}:
                item["explanation"] = f"{unit['title']}: “{item['answer']['value']}” comunica a ideia pedida em {name}."
