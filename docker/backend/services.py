...
class ExerciseService:
    ...
    SESSION_SIZE = 20
    TARGET_ITEMS = 1000
    INCREMENTAL_ITEM_TARGETS = {"de": 1220, "fr": 1130, "ru": 1130, "jp": 1130, "en": 1130}
    JP_BEGINNER_KANA = {
    ...
    def _incremental_review_items(code: str, start_index: int, count: int):
        ...
        unit = A1_UNITS[7]
        session_57_start = len(items)
        phrases = unit["phrases"][code]
        prefix = "Sessão 57 — Revisão incremental · Roupas em contexto"
        options = [foreign for _pt, foreign in phrases]
        portuguese_options = [pt for pt, _foreign in phrases]
        ...
        pt, target = phrases[4]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — descreva uma peça de roupa. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[5:8]],
            start_index + len(items),
        ))
        ...
        sequence_pairs = [phrases[4], phrases[5], phrases[6], phrases[7]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro cor; depois tamanho; em seguida preço; por fim provar",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        listen_pairs = [[foreign, portuguese] for portuguese, foreign in [phrases[0], phrases[2], phrases[4], phrases[9]]]
        items.append(ExerciseService._listen_match(
            f"{prefix}: ouça cada áudio em {name} e selecione a tradução em português",
            listen_pairs,
            start_index + len(items),
        ...
        sequence_pairs = [phrases[0], phrases[4], phrases[6], phrases[8]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro peça; depois cor; em seguida preço; por fim compra",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[5]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — peça um tamanho menor. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
        ...
        for item in items[session_57_start:]:
            if item["type"] == "listen_build":
                ...
                item["hint"] = f"{hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
                item["explanation"] = f"Cada áudio em {name} deve ser ligado à tradução em português dentro da revisão de roupas."
            elif item["type"] == "sequence_dialogue":
                item["hint"] = f"{hint} Siga a ordem indicada no enunciado e organize apenas as frases no idioma estudado."
            else:
                item["hint"] = hint
        ...
        unit = A1_UNITS[8]
        session_58_start = len(items)
        phrases = unit["phrases"][code]
        prefix = "Sessão 58 — Revisão incremental · Hábitos em contexto"
        options = [foreign for _pt, foreign in phrases]
        ...
        pt, target = phrases[4]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — descreva uma rotina diária. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[5:8]],
            start_index + len(items),
        ))
        ...
        sequence_pairs = [phrases[4], phrases[5], phrases[6], phrases[7]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro almoço; depois exercício; em seguida noite; por fim fim de semana",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[6]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — fale sobre o hábito da noite. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
        ...
        for item in items[session_58_start:]:
            if item["type"] == "listen_build":
                ...
                item["hint"] = f"{hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
                item["explanation"] = f"Cada áudio em {name} deve ser ligado à tradução em português dentro da revisão de hábitos."
            elif item["type"] == "sequence_dialogue":
                item["hint"] = f"{hint} Siga a ordem indicada no enunciado e organize apenas as frases no idioma estudado."
            else:
                item["hint"] = hint
        ...
        image_sample = [phrases[8], phrases[9], phrases[0], phrases[1]]
        answer_pt, answer_foreign = image_sample[0]
        items.append(ExerciseService._image_choice(
            f"{prefix}: observe a imagem e escolha a frase que representa “{answer_pt}”",
            (answer_pt, answer_foreign, ExerciseService._icon_key_for_phrase(answer_pt, answer_foreign, unit["topics"][8])),
            [(pt, foreign, ExerciseService._icon_key_for_phrase(pt, foreign, unit["topics"][8])) for pt, foreign in image_sample[1:]],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[0]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — descreva um hábito diário. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[1:4]],
            start_index + len(items),
        ))

        for item in items[session_58_start + 10:]:
            item["hint"] = hint
            if item["type"] in {"build", "listen_build"}:
                item["explanation"] = f"A frase correta é: “{' '.join(item['answer']['value'])}”."
            elif item["type"] not in {"image_choice", "listen_match", "sequence_dialogue"}:
        ...
        listen_pairs = [[foreign, portuguese] for portuguese, foreign in [phrases[6], phrases[7], phrases[8], phrases[9]]]
        items.append(ExerciseService._listen_match(
            f"{prefix}: ouça cada áudio em {name} e selecione a tradução em português",
            listen_pairs,
            start_index + len(items),
        ...
        pt, target = phrases[8]
        wrong_portuguese = [option for option in [phrases[4][0], phrases[5][0], phrases[6][0], phrases[7][0]] if option != pt][:3]
        items.append(ExerciseService._choice(
            f"{prefix}: entenda “{target}” — qual é o significado em português?",
            pt,
            wrong_portuguese,
        ...
        sequence_pairs = [phrases[6], phrases[7], phrases[8], phrases[9]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro noite; depois fim de semana; em seguida frequência; por fim às vezes",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[1]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — diga o horário do café da manhã. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[2:5]],
            start_index + len(items),
        ))

        for item in items[session_58_start + 15:]:
            if item["type"] == "listen_build":
                item["hint"] = f"{hint} Ouça a frase, repita em voz alta e monte as palavras na ordem correta."
            elif item["type"] == "listen_match":
                item["hint"] = f"{hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
        ...
        unit = A1_UNITS[-1]
        session_59_start = len(items)
        phrases = unit["phrases"][code]
        prefix = "Sessão 59 — Revisão incremental · Preferências em contexto"
        options = [foreign for _pt, foreign in phrases]
        ...
        pt, target = phrases[4]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — fale de preferência musical. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[5:8]],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — feche uma opinião simples sobre preferências. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))

        for item in items[session_59_start + 10:]:
            item["hint"] = hint
            if item["type"] in {"build", "listen_build"}:
                item["explanation"] = f"A frase correta é: “{' '.join(item['answer']['value'])}”."
            elif item["type"] not in {"image_choice", "listen_match", "sequence_dialogue"}:
        ...
        listen_pairs = [[foreign, portuguese] for portuguese, foreign in [phrases[0], phrases[1], phrases[2], phrases[3]]]
        items.append(ExerciseService._listen_match(
            f"{prefix}: ouça cada áudio em {name} e selecione a tradução em português",
            listen_pairs,
            start_index + len(items),
        ...
        pt, target = phrases[1]
        wrong_portuguese = [option for option in [phrases[0][0], phrases[2][0], phrases[3][0], phrases[4][0]] if option != pt][:3]
        items.append(ExerciseService._choice(
            f"{prefix}: entenda “{target}” — qual é o significado em português?",
            pt,
            wrong_portuguese,
        ...
        sequence_pairs = [phrases[0], phrases[2], phrases[5], phrases[9]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro gosto; depois preferência; em seguida filme; por fim opinião",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — conclua a conversa com uma opinião curta. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))

        for item in items[session_59_start + 15:]:
            if item["type"] == "listen_build":
                item["hint"] = f"{hint} Ouça a frase, repita em voz alta e monte as palavras na ordem correta."
            elif item["type"] == "listen_match":
                item["hint"] = f"{hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
        ...
        unit = A1_UNITS[0]
        session_60_start = len(items)
        phrases = unit["phrases"][code]
        prefix = "Sessão 60 — Revisão incremental · Café em contexto"
        options = [foreign for _pt, foreign in phrases]
        ...
        items.append(ExerciseService._listen_choice(
            f"{prefix}: ouça o áudio e identifique a fala que comunica “{pt}”",
            target,
            [foreign for foreign in options[3:6]],
            start_index + len(items),
        ))

        image_sample = [phrases[3], phrases[1], phrases[2], phrases[7]]
        answer_pt, answer_foreign = image_sample[0]
        items.append(ExerciseService._image_choice(
            f"{prefix}: observe a imagem e escolha a frase que representa “{answer_pt}”",
            (answer_pt, answer_foreign, ExerciseService._icon_key_for_phrase(answer_pt, answer_foreign, unit["topics"][3])),
            [(pt, foreign, ExerciseService._icon_key_for_phrase(pt, foreign, unit["topics"][3])) for pt, foreign in image_sample[1:]],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[8]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — confirme um pedido simples no café. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))

        listen_pairs = [[foreign, portuguese] for portuguese, foreign in [phrases[4], phrases[5], phrases[6], phrases[7]]]
        items.append(ExerciseService._listen_match(
            f"{prefix}: ouça cada áudio em {name} e selecione a tradução em português",
            listen_pairs,
            start_index + len(items),
        ...
        pt, target = phrases[9]
        wrong_portuguese = [option for option in [phrases[0][0], phrases[1][0], phrases[2][0], phrases[3][0]] if option != pt][:3]
        items.append(ExerciseService._choice(
            f"{prefix}: entenda “{target}” — qual é o significado em português?",
            pt,
            wrong_portuguese,
        ...
        sequence_pairs = [phrases[1], phrases[2], phrases[7], phrases[9]]
        items.append(ExerciseService._sequence_dialogue(
            f"{prefix}: prática guiada de ordem — organize os cartões exatamente assim: primeiro café; depois água; em seguida conta; por fim despedida",
            [foreign for _portuguese, foreign in sequence_pairs],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — encerre o atendimento no café. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
        ...
        items.append(ExerciseService._choice(
            f"{prefix}: escolha como dizer “{pt}” em {name}",
            target,
            [foreign for foreign in [options[0], options[1], options[3]]],
            start_index + len(items),
        ))
        ...
        items.append(ExerciseService._listen_choice(
            f"{prefix}: ouça o áudio e identifique a fala que comunica “{pt}”",
            target,
            [foreign for foreign in [options[1], options[2], options[4]]],
            start_index + len(items),
        ))

        image_sample = [phrases[6], phrases[1], phrases[2], phrases[8]]
        answer_pt, answer_foreign = image_sample[0]
        items.append(ExerciseService._image_choice(
            f"{prefix}: observe a imagem e escolha a frase que representa “{answer_pt}”",
            (answer_pt, answer_foreign, ExerciseService._icon_key_for_phrase(answer_pt, answer_foreign, unit["topics"][6])),
        ...
        pt, target = phrases[5]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — agradeça no atendimento do café. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
        listen_pairs = [[foreign, portuguese] for portuguese, foreign in [phrases[1], phrases[2], phrases[7], phrases[9]]]
        items.append(ExerciseService._listen_match(
            f"{prefix}: ouça cada áudio em {name} e selecione a tradução em português",
            listen_pairs,
            start_index + len(items),
        ...
        pt, target = phrases[7]
        wrong_portuguese = [option for option in [phrases[1][0], phrases[2][0], phrases[5][0], phrases[9][0]] if option != pt][:3]
        items.append(ExerciseService._choice(
            f"{prefix}: entenda “{target}” — qual é o significado em português?",
            pt,
            wrong_portuguese,
        ...
        for item in items[session_60_start:]:
            if item["type"] == "listen_build":
                ...
                item["hint"] = f"{hint} Toque em cada áudio no idioma estudado e selecione a tradução correspondente em português."
                item["explanation"] = f"Cada áudio em {name} deve ser ligado à tradução em português dentro da revisão de café."
            elif item["type"] == "sequence_dialogue":
                item["hint"] = f"{hint} Siga a ordem indicada no enunciado e organize apenas as frases no idioma estudado."
            else:
                item["hint"] = hint
        ...
        image_sample = [phrases[2], phrases[0], phrases[1], phrases[8]]
        answer_pt, answer_foreign = image_sample[0]
        items.append(ExerciseService._image_choice(
            f"{prefix}: observe a imagem e escolha a frase que representa “{answer_pt}”",
            (answer_pt, answer_foreign, ExerciseService._icon_key_for_phrase(answer_pt, answer_foreign, unit["topics"][2])),
        ...
        pt, target = phrases[4]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — apresente sua profissão de forma simples. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[5:8]],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[6]
        items.append(ExerciseService._choice(
            f"{prefix}: escolha como perguntar “{pt}” em {name}",
            target,
            [foreign for foreign in [options[0], options[7], options[9]]],
            start_index + len(items),
        ))

        pt, target = phrases[7]
        items.append(ExerciseService._listen_choice(
            f"{prefix}: ouça o áudio e identifique a pergunta que comunica “{pt}”",
            target,
            [foreign for foreign in [options[1], options[6], options[9]]],
            start_index + len(items),
        ))

        image_sample = [phrases[8], phrases[0], phrases[4], phrases[9]]
        answer_pt, answer_foreign = image_sample[0]
        items.append(ExerciseService._image_choice(
            f"{prefix}: observe a imagem e escolha a frase que representa “{answer_pt}”",
            (answer_pt, answer_foreign, ExerciseService._icon_key_for_phrase(answer_pt, answer_foreign, unit["topics"][8])),
        ...
        pt, target = phrases[5]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — diga qual idioma você está estudando. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in [options[0], options[4], options[8]]],
            start_index + len(items),
        ))
        ...
        pt, target = phrases[6]
        wrong_portuguese = [option for option in [phrases[1][0], phrases[3][0], phrases[5][0], phrases[9][0]] if option != pt][:3]
        items.append(ExerciseService._choice(
            f"{prefix}: entenda “{target}” — qual é o significado em português?",
            pt,
            wrong_portuguese,
        ...
        pt, target = phrases[9]
        items.append(ExerciseService._context_choice(
            f"{prefix}: situação guiada — encerre a apresentação de forma educada. Escolha a fala que comunica “{pt}” em {name}.",
            target,
            [foreign for foreign in options[0:3]],
            start_index + len(items),
        ))
