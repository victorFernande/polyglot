...
def test_incremental_cron_target_closes_active_german_session_54_with_twenty_contact_items():
    ...
    assert ExerciseService.target_items_for_language("de") == 1100
...
def test_incremental_cron_target_closes_active_german_session_55_with_twenty_family_items():
    assert ExerciseService.SESSION_SIZE == 20
    assert ExerciseService.TARGET_ITEMS == 1000
    assert ExerciseService.target_items_for_language("de") == 1100
    assert {language: ExerciseService.target_items_for_language(language) for language in LANGUAGES - {"de"}} == {
    ...
    german_items = ExerciseService.generate_items("de")
    last_block_size = len(german_items) % ExerciseService.SESSION_SIZE
    session_55 = german_items[1080:1100]
    session_55_second_half = german_items[1090:1100]

    assert len(german_items) == 1100
    assert last_block_size == 0
    assert len(session_55) == ExerciseService.SESSION_SIZE
    assert len(session_55_second_half) == 10
    assert [item["type"] for item in session_55_second_half] == [
        "choice",
        "listen_choice",
        "image_choice",
        "build",
    ...
    assert all("Sessão 55" in item["prompt"] for item in session_55)
    assert any("Mein Sohn ist hier." in repr(item) for item in session_55_second_half)
    assert any("Meine Tochter ist klein." in repr(item) for item in session_55_second_half)
    assert any("Meine Mutter ist fünfzig Jahre alt." in repr(item) for item in session_55_second_half)
    assert any("Das ist mein Haus." in repr(item) for item in session_55_second_half)
    assert all("a palavra" not in item["prompt"].casefold() for item in session_55)
    assert all("das Wort" not in repr(item) for item in session_55)
    sequence = session_55_second_half[-2]
    assert sequence["type"] == "sequence_dialogue"
    assert sequence["options"] is None
    assert sequence["pairs"] is None
    assert "português" not in "\n".join(sequence["tiles"]).casefold()
...
def test_previous_incremental_german_session_55_first_half_remains_unchanged():
    german_items = ExerciseService.generate_items("de")
...
def test_seed_lessons_appends_incremental_items_without_replacing_existing_ids():
    ...
    try:
        ...
        assert [item.order_index for item in items[-10:]] == list(range(1091, 1101))
